...
def simulate_sir(beta=0.45, gamma=0.2, I0=1e-5, steps=104):
    S = np.zeros(steps)
    I = np.zeros(steps)
    R = np.zeros(steps)
...
class SFNN(nn.Module):
    def __init__(self, p:int, n:int, hidden:int, depth:int, h:int):
        ...
        layers = [
            nn.Flatten(),
            nn.Linear(p * n, hidden),
            nn.BatchNorm1d(hidden),     # ✅ Added BatchNorm
            nn.ReLU(),
            nn.Dropout(0.1)
        ]
    ...
    def forward(self, x):
        return self.net(x).view(-1, self.h, self.n)
...
def train(model, tensor, args):
    ...
    for ep in range(args.epochs):
        ...
        if (ep+1)%5==0:
            ...
            print(f"Ep {ep+1:>3}/{args.epochs} train {tot/len(tr):.4f} val {vtot/len(vl):.4f}")
...
def main(argv: List[str]|None=None):
    ...
    ap.add_argument("--epochs", type=int, default=100)  # ✅ recommended
    ap.add_argument("--lr", type=float, default=1e-4)   # ✅ recommended
    ...
    sir_series = simulate_sir(steps=T.shape[0])  # shape [T]
    sir_matrix = np.tile(sir_series[:, None], (1, T.shape[1]))  # shape [T, N]
    T_aug = np.concatenate([T, sir_matrix], axis=1)  # shape [T, 2N]
    tensor = torch.tensor(T_aug, dtype=torch.float32)


    model = SFNN(args.window, T_aug.shape[1],
                 args.hidden, args.depth, args.horizon).to(args.device)
    train(model, tensor, args)

    preds = forecast(model, tensor, args)
    n_states = len(states)
    preds_trimmed = preds[:, :n_states]  # ✅ 修正维度

    pd.DataFrame(preds_trimmed, columns=states)\
