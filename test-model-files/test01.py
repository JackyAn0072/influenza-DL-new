...
class FluDataset(torch.utils.data.Dataset):
    ...
    def _window(t: torch.Tensor, p: int, h: int):
        ...
        for idx in range(p, t.shape[0] - h + 1):
            xs.append(t[idx - p:idx])
            ys.append(t[idx:idx + h])
...
def main(argv: List[str]|None=None):
    ...
    for _,r in weekly.iterrows():
        if r["state"] in s2c:
            val = np.log1p(r["value"])  # log1p transform
            T[w2r[r["epiweek"]], s2c[r["state"]]] = val

    sir_series = simulate_sir(steps=T.shape[0])
    sir_matrix = np.tile(sir_series[:, None], (1, T.shape[1]))
    T_aug = np.concatenate([T, sir_matrix], axis=1)
    ...
    n_states = len(states)
    preds_trimmed = preds[:, :n_states] 
    preds_exp = np.expm1(preds_trimmed)  # inverse log transform

    pd.DataFrame(preds_exp, columns=states)\
